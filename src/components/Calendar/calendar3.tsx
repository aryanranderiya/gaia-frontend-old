...
export default function Calendar() {
  ...
  const addOrUpdateEvent = () => {
    ...
    setNewEvent({ title: '', start: new Date(), end: new Date(), categoryId: 'personal', isAllDay: false })
  ...
  const parseNaturalLanguage = () => {
    ...
    words.forEach((word, index) => {
      ...
      } else if (word === 'allday' || (word === 'all' && words[index + 1] === 'day')) {
    ...
    newEvent.title = words.filter(w => !['tomorrow', 'at', 'work', 'personal', 'family', 'other', 'allday', 'all', 'day'].includes(w)).join(' ')
  ...
  const handleNaturalLanguageKeyDown = (e: ReactKeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
                ...
                >
                  {events
                    ...
                    .map(event => (
                      ...
                      >
                        {event.title} {!event.isAllDay && `(${event.start.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - ${event.end.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})})`}
                      </div>
                    ))
                  }
                </div>
            ...
            >
              ...
              {events
                ...
                .map(event => (
                  ...
                  >
                    {event.title} {!event.isAllDay && `(${event.start.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})})`}
                  </div>
                ))
              }
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                value={newEvent.isAllDay ? newEvent.start.toISOString().split('T')[0] : newEvent.start.toISOString().slice(0, 16)}
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                id="end"
                type={newEvent.isAllDay ? "date" : "datetime-local"}
                value={newEvent.isAllDay ? newEvent.end.toISOString().split('T')[0] : newEvent.end.toISOString().slice(0, 16)}
